...
### Exit criteria
- [ ] FS plugin loads from a signed `.wasm` at startup.
- [ ] Unsigned `.wasm` is refused with a visible reason.
...
- [ ] Path canonicalization covers symlink escape (proptest fixture).
- [ ] Permission cache invalidates on user edit.
