...
### JavaScript plugin (language-neutral proof)

`plugins/hello-js` is a plugin written in **JavaScript** (not Rust) that compiles to the same WASM Component format via [`jco componentize`](https://github.com/bytecodealliance/jco). It implements the same `rustacle:plugin/module` WIT interface.

This proves the architecture claim: **any language** that targets the WebAssembly Component Model can be a Rustacle plugin.
...
| Plugin | Language | Size (debug) | Commands |
|--------|----------|-------------|----------|
| `plugins/fs` | Rust | 4.3 MB | read_file, list_dir, stat |
| `plugins/hello-js` | JavaScript | 12 MB | greet, ping, info |

Both pass the same wasmtime component type-check (verified in integration tests).
